...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check tax 1
    if (trim($_POST['tax1']) != '' && !is_numeric($_POST['tax1'])) {
        $error['tax1'] = 'Tax 1 must be blank or numeric';
    } else {
        $tax_1 = filter_var($_POST['tax1'], FILTER_SANITIZE_STRING);
    }
    
    // Check tax 2
    if (trim($_POST['tax2']) != '' && !is_numeric($_POST['tax2'])) {
        $error['tax2'] = 'Tax 2 must be blank or numeric';
    } else {
        $tax_2 = filter_var($_POST['tax2'], FILTER_SANITIZE_STRING);
    }
    ...
    if ($insert) {
        ...
        if (!empty($_FILES['ex_file']['name'])) {
            ...
            unlink($uploaddir . '/' . $_POST['att_file']);
        ...
        $sql = "
            ...
            echo 'go';
        if (mysql_query($sql)) {
            header("Location:$modulelink&expense=list&msgid=14");
            exit();
        } else {
            header("Location:$modulelink&expense=list&msgid=15");
        }
    }
}

if ((isset($insert) && !$insert) || $_SERVER['REQUEST_METHOD'] != 'POST') {
    $id = filter_var($_REQUEST['id'], FILTER_SANITIZE_STRING);
    ...
    $date = date($date_format_insert, $expense['date']);
    ...
    $tax_1 = $expense['tax1'];
    $tax_2 = $expense['tax2'];
    $inclusive = $expense['inclusive'];

    $recurring = $expense['recurring'];
    $freq = $expense['frequency'];
    $untill = $expense['untill'];
  
    $end_date = ($expense['end_date'] == 0 || trim($expense['end_date']) == '') ? '' : date($date_format_insert, $expense['end_date']);
    ...
    /**
     * WHMCS API Connection to fetch list of clients
     */
    $clients_exist = false;
    ...
    if ($clientresult['numreturned'] > 0) {
        foreach ($clientresult['clients']['client'] as $client) {
            if ($expense['client_id'] == $client['id']) {
    ...
    <fieldset style="margin: 0 auto; width: 800px; border-color:#f7f7f7;">
        <form name="expense_add" method="post" enctype="multipart/form-data" action="<?php echo $modulelink ?>&expense=edit">
                ...
                <tr valign="top">
                    <td>
                        <table width="100%" border="0">
                            <tr>
                                <td valign="top" class="text_newproject">Edit Expense</td>
                            </tr>
                            <tr>
                                ...
                                <td valign="top">
                                    <table width="100%" border="0" cellspacing="0">
                                        <?php /* <tr>
                                            ...
                                            <td width="79%" class="text_project1">
                                                <select  name="currency">
                                                    <?php
                                                    for ($i = 0; $i < count($results['currencies']['currency'][$i]); $i++) {
                                                        if ($results['currencies']['currency'][$i]['prefix'] == $currency) {
                                                            $select = 'selected="selected"';
                                                        } else {
                                                            $select = '';
                                                        }
                                                        ?>
                                                        <option <?php echo $select; ?>value="<?php echo $results['currencies']['currency'][$i]['prefix']; ?>" ><?php echo $results['currencies']['currency'][$i]['prefix'] . ' ' . $results['currencies']['currency'][$i]['code']; ?></option>
                                                    <?php } ?>
                                                </select>
                                            </td>
                                        </tr> */ ?>
                                        ...
                                        <tr>
                                            ...
                                            <td width="79%" class="text_project1">
                                                <select name="vendor_list" id="vendor_list">
                                                    <?php
                                                    echo $vendor_select;
                                                    ?>
                                                </select>
                                        ...
                                        <tr>
                                            <td width="21%" class="textname_box">Fee</td>
                                            <td width="79%" class="text_project1">
                                        ...
                                        <tr>
                                            ...
                                            <td width="79%" class="text_project1">
                                                <input type="file" name="ex_file" id="ex_file"><input type="hidden" name="att_file" value="<?php echo $expense['attachment']; ?>"/><span style="margin-left: 100px;"> <?php echo $expense['attachment']; ?></span>
                                            </td>
                                        </tr>                                        
                                    </table>
                                </td>
                                ...
                                <td valign="top">
                                    <table width="100%" border="0" cellspacing="0">
                                        <tr>
                                            ...
                                            <td width="79%" class="text_project1">
                                                <input type="text" name="tax1" value="<?= $tax_1 ?>" id="tax1"/>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td width="21%" class="textname_box">Tax Level 2 %</td>
                                            <td width="79%" class="text_project1">
                                                <input type="text" name="tax2" value="<?= $tax_2 ?>" id="tax2"/>
                                            </td>
                                        </tr>    
                                        <tr>
                                            <td width="21%" class="textname_box">Inclusive </td>
                                            <td width="79%" class="text_project1">
                                                <input type="checkbox" name="inclusive" value="1" <?php echo ($inclusive == 1 ? 'checked="checked"' : ''); ?> />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                ...
                                <td valign="top">
                                    <table width="100%" border="0" cellspacing="0">
                                        ...
                                        <tr>
                                            ...
                                            <td width="79%" class="text_project1">
                                                <input type="text" name="end_date" id="end_date" value="<?php echo ($end_date == 0 || $end_date == '943912800' || trim($end_date) == '') ? "" : $end_date; ?>" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
                                ...
                                <td valign="top">
                                    <table width="100%" border="0" cellspacing="0">
                                        ...
                                        <tr>
                                            ...
                                            <td class="text_project1">
                                                <br />
                                                <input type="submit" name="submit" value="Edit Expense" />
                                            </td>
                                        </tr>
                                    </table>
                                </td>
